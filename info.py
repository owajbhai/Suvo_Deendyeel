...
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002037872585').split()] #Channel id for auto indexing ( make sure bot is admin )
LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1002158259855')) #Log channel id ( make sure bot is admin )
...
PREMIUM_LOGS = int(environ.get('PREMIUM_LOGS', '-1002158259855')) # add premium logs channel id
...
DEENDAYAL_VERIFIED_LOG = int(environ.get('DEENDAYAL_VERIFIED_LOG', '-1002158259855')) #Log channel id ( make sure bot is admin )
